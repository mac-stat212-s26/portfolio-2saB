...
month_starts <- seq(
  from = floor_date(min(weather$date), "month"),
  to   = ceiling_date(max(weather$date), "month"),
  by   = "1 month"
)
...
```{r}
month_centers <- month_starts[-length(month_starts)] +
  (month_starts[-1] - month_starts[-length(month_starts)]) / 2

month_labels <- data.frame(
  x = month_centers,
  label = month.abb,
  y = min(weather$RecordLow, na.rm = TRUE) + 2
)
...
weather %>%
  mutate(date = as.Date(date),
         year = format(date, "%Y")) -> weather2
  # select(Record = TRUE)     -> weather2

ggplot(weather2, aes(x = dateInYear, y = CulmPrec, group = year)) +
  geom_area(aes(fill = year), fill = "#ebeae2") +
  geom_line(aes(color = year), color = "#32a3d8") +
  geom_point(aes(RecordP), shape = 25) +
  theme_classic()+
  labs(
    x = "Day of Year",
    y = "Cumulative Precipitation",
    title = "Cumulative Precipitation by Day of Year"
  ) 
