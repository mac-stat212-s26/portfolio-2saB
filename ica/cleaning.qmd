...
weather <- read_csv("./weather data file.csv")

#  Clean nonsensical values
# weather <- weather %>%
#   mutate(
#     RecordP = na_if(RecordP, -999)
#   )

# 3️⃣ Create `dateInYear` (day of the year)
weather <- weather %>%
  mutate(
    date = as.Date(date, format = "%m/%d/%Y"),       
    dateInYear = yday(date)     # day of year (1-365)
  )

# 4️⃣ Create `month_name` (3-letter abbreviation)
weather <- weather %>%
  mutate(
...
dir.create("portfolio-2saB/ica/", recursive = TRUE, showWarnings = FALSE)

write_csv(
  weather,
  "portfolio-2saB/ica/weather_clean.csv"
)
